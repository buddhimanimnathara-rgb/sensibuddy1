...
class _FindItScreenState extends State<FindItScreen> {
  ...
  final List<GameItem> _items = const [
    GameItem(
      id: 'apple',
      emoji: '🍎',
      name: 'Apple',
  ...
  void _startGame() {
    final shuffledQuestions = List<GameItem>.from(_items)
      ..shuffle(_random);
  ...
  Future<void> _selectItem(GameItem selected) async {
    if (_isLocked ||
        _isFinished ||
        _questions.isEmpty) {
      return;
  ...
  Future<void> _saveResult() async {
    ...
    try {
      ...
      debugPrint(
        'Find It analytics saved successfully',
      );
    } catch (e) {
      debugPrint(
        'Find It save error: $e',
      );
    } finally {
      _isSaving = false;
    }
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const Text(
              'Look carefully and find the correct item!',
              textAlign: TextAlign.center,
              style: TextStyle(
                fontSize: 16,
                color: Colors.black54,
